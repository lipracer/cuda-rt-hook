...
HOOK_API bool call_python_method(PyObject* py_instance, HookString_t name,
                                 HookString_t value) {
    Py_Initialize();
...
HOOK_API void create_hook_installer(PyObject* py_instance, HookString_t lib) {
    auto is_target = [=](HookString_t name) {
        return call_python_method(py_instance, "is_target_lib", name);
    };
    auto is_symbol = [=](HookString_t name) {
        return call_python_method(py_instance, "is_target_symbol", name);
    };
    dh_create_py_hook_installer(is_target, is_symbol, lib);
}
}
